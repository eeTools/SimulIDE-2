...
AvrCore::AvrCore( eMcu* mcu )
       : McuCpu( mcu )
{
    McuRegister* eind = m_mcuRam->getRegByName("EIND");
    if( eind ) EIND = eind->getData08();
    else       EIND = nullptr;

    McuRegister* rampz = m_mcuRam->getRegByName("RAMPZ");
    if( rampz )
    {
        RAMPZ = rampz->getData08();
        m_rampzAddr = rampz->getAddress();
    }
    else RAMPZ = nullptr;
...
{
    uint16_t o = m_mcuPgm->read_16( pc ) & 0xfc0f;
...
{
    m_mcu->cyclesDone = 0;
    uint16_t instruction = m_mcuPgm->read_16( m_PC );
    ...
    switch( instruction & 0xf000 )
    {
        ...
        case 0x8000: {
            ...
            switch( instruction & 0xd008 )
            {
                ...
                case 0x8000: {    // LD( LDD) -- Load Indirect using Z -- 10q0 qqsd dddd yqqq
                    v = m_mcuRam->read_16( (R_ZH << 8) | R_ZL );
                }    break;
                case 0xa008:
                case 0x8008: {    // LD( LDD) -- Load Indirect using Y -- 10q0 qqsd dddd yqqq
                    v = m_mcuRam->read_16( (R_YH << 8) | R_YL );
                }    break;
                //default: ;//_avr_invalid_instruction(avr);
            }
            get_d5_q6( instruction );
            if( instruction & 0x0200) m_mcuRam->write_08( v+q, GET_RAM(d) );
            else                      m_mcuRam->write_08( d, GET_RAM(v+q) );
        ...
        case 0x9000: {
            ...
            if( (instruction & 0xff0f) == 0x9408 )
            {
                ...
                write_S_Bit( bit, set );
                if( bit == S_I ) m_mcu->interrupts()->enable( set );
            }
            switch( instruction )
            {
...
////////             if( !int_pending.empty() || !SREG[S_I]) state = cpu_Sleeping;
                ...
                case 0x9519: { // EICALL -- Indirect Call to Subroutine -- 1001 0101 0001 1001   bit 8 is "Call: push pc"
                    ...
                    uint32_t z = m_mcuRam->read_16( (R_ZH << 8) | R_ZL );
                    if( exte ){
                        if( !EIND ){
                ...
                case 0x9508: {   // RET -- Return -- 1001 0101 0000 1000
                    new_pc = POP_STACK();
                    cycle += 1 + m_progAddrBytes;
                }    break;
                case 0x95c8: {    // LPM -- Load Program Memory R0 <-( Z) -- 1001 0101 1100 1000
                    uint16_t z = m_mcuRam->read_16( (R_ZH << 8) | R_ZL );
                    cycle += 2; // 3 cycles
                    uint16_t prgData = m_mcuPgm->read_16( z/2 );
                    if( z&1 ) prgData >>= 8;
                    m_mcuRam->write_08( 0, prgData );
                }    break;
                case 0x95d8: {    // ELPM -- Load Program Memory R0 <-( Z) -- 1001 0101 1101 1000
                    ...
                    uint32_t z = m_mcuRam->read_16( (R_ZH << 8) | R_ZL ) | (*RAMPZ << 16);
                    uint16_t prgData = m_mcuPgm->read_16( z/2 );
                    if( z&1 ) prgData >>= 8;
                    m_mcuRam->write_08( 0, prgData );
                    cycle += 2; // 3 cycles
                ...
                default:  {
                    switch( instruction & 0xfe0f) {
                        case 0x9000: {    // LDS -- Load Direct from Data Space, 32 bits -- 1001 0000 0000 0000
                            get_d5( instruction );
                            uint16_t x = m_mcuPgm->read_16( new_pc );
                            new_pc += 1;
                            m_mcuRam->write_08( d, GET_RAM(x) );
                            cycle++; // 2 cycles
                        }    break;
                        case 0x9005:
                        ...
                        case 0x9007: {    // ELPM -- Extended Load Program Memory -- 1001 000d dddd 01oo
                            ...
                            uint16_t z = m_mcuRam->read_16( (R_ZH << 8) | R_ZL ) | (*RAMPZ << 16);
                            get_d5( instruction );
                            int op = instruction & 1;
                            uint16_t prgData = m_mcuPgm->read_16( z/2 );
                            if( z&1 ) prgData >>= 8;
                            ...
                            if( op) {
                                z++;
                                m_mcuRam->write_08( m_rampzAddr, z>>16 );
                                SET_REG16_HL( R_ZL, z );
                            }
                            cycle += 2; // 3 cycles
                        }    break;
                        ...
                        case 0x900a: {    // LD -- Load Indirect from Data using Y -- 1001 000d dddd 10oo
                            int op = instruction & 3;
                            get_d5( instruction );
                            uint16_t y = m_mcuRam->read_16( (R_YH << 8) | R_YL );
                            cycle++; // 2 cycles, except tinyavr
                            if( op == 2) y--;
                        ...
                        case 0x9202: {    // ST -- Store Indirect Data Space Z -- 1001 001d dddd 00oo
                            ...
                            uint16_t z = m_mcuRam->read_16( (R_ZH << 8) | R_ZL );
                             cycle++; // 2 cycles, except tinyavr
                            if( op == 2) z--;
                            m_mcuRam->write_08( z, vd );
                            if( op == 1 ) z++;
                            SET_REG16_HL( R_ZL, z);
                        }    break;
                        case 0x900f: {    // POP -- 1001 000d dddd 1111
                            get_d5( instruction );
                            m_mcuRam->write_08( d, POP_STACK8() );
                            cycle++;
                        }    break;
        ...
        case 0xb000: {
            switch( instruction & 0xf800) {
                case 0xb800: {    // OUT A,Rr -- 1011 1AAd dddd AAAA
                    ...
                    m_mcuRam->write_08( A, GET_RAM(d) );
                }    break;
                case 0xb000: {    // IN Rd,A -- 1011 0AAd dddd AAAA
                    get_d5_a6( instruction );
                    m_mcuRam->write_08( d, GET_RAM(A) );
                }    break;
                default: ;//_avr_invalid_instruction(avr);
            }
        }    break;
