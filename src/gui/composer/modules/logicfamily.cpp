...
void LogicFamily::setSupplyV( double v )
{
    if( v < 0 ) v = 0;
    m_supplyV = v;
...
void LogicFamily::setPropDelay( uint64_t pd )
{
    //if( pd < 0 ) pd = 0;
    //if( pd > 1e6   ) pd = 1e6;
    m_delayBase = pd;
...
void LogicFamily::setRiseTime( uint64_t time )
{
...
void LogicFamily::setFallTime( uint64_t time )
{
    //if( time < 1e-12 ) time = 1e-12;
    //if( time > 1e6   ) time = 1e6;
    m_timeHL = time;
